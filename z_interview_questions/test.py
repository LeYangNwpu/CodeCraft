...
# in_data_file = 'data.in'
# out_data_file = 'data.out'
...
for itest in range(num_all):
    N_strs_line = input()
    data_in_sets = N_strs_line.split(' ')
    max_hack = int(data_in_sets[0])
    strs = list(data_in_sets[1])
    step = save_universe(max_hack, strs[:-1])
    if step == -1:
        step = 'IMPOSSIBLE'
    order = itest + 1
    print('Case #{}: {}\n'.format(order, step))
